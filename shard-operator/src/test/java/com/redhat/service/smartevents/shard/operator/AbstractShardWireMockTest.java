...
public abstract class AbstractShardWireMockTest extends AbstractWireMockTest {
    ...
    protected void stubProcessorsToDeployOrDelete(List<ProcessorDTO> processorDTOS) throws JsonProcessingException {
        wireMockServer.stubFor(get(urlEqualTo(APIConstants.V1_SHARD_API_BASE_PATH + "processors"))
    ...
    protected void stubBridgesToDeployOrDelete(List<BridgeDTO> bridgeDTOs) throws JsonProcessingException {
        wireMockServer.stubFor(get(urlEqualTo(APIConstants.V1_SHARD_API_BASE_PATH))
                .willReturn(aResponse()
                        .withHeader("Content-Type", "application/json")
    ...
    protected void stubProcessorUpdate() {
        wireMockServer.stubFor(put(urlEqualTo(APIConstants.V1_SHARD_API_BASE_PATH + "processors"))
                .willReturn(aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withStatus(200)));
    }
    ...
    protected void stubBridgeUpdate() {
        wireMockServer.stubFor(put(urlEqualTo(APIConstants.V1_SHARD_API_BASE_PATH))
                .willReturn(aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withStatus(200)));
    }
    ...
    protected void addProcessorUpdateRequestListener(CountDownLatch latch) {
        addUpdateRequestListener(APIConstants.V1_SHARD_API_BASE_PATH + "processors", RequestMethod.PUT, latch);
    }

    protected void addBridgeUpdateRequestListener(CountDownLatch latch) {
        addUpdateRequestListener(APIConstants.V1_SHARD_API_BASE_PATH, RequestMethod.PUT, latch);
    }
}
