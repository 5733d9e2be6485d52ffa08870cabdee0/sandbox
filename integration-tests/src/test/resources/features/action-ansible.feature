Feature: Ansible Action tests
  ...
  Scenario: Job template is correctly run
    ...
    """
    {
      ...
      "transformationTemplate": "{\"extra_vars\":{\"name_to_greet\": {data}}}"
    }
    """
    ...
    """
    {
      ...
      "id": "ansible-test",
      "data": "${uuid.name.to.greet}"
    }
    """
    Then Ansible job with the extra parameter "${uuid.name.to.greet}" was run using template "Greetings" within 2 minutes
