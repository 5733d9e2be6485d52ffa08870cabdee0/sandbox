...
public class PerformanceSteps {
    ...
    public void runBenchmarkOnHyperfoilWithinMinutes(String perfTestName, int timeoutMinutes) {
        this.benchmarkRuns.put(perfTestName, HyperfoilResource.runBenchmark(context.getManagerToken(), perfTestName));
        context.getScenario().log("Running benchmark ID " + benchmarkRuns.get(perfTestName));

        Awaitility.await()
                .conditionEvaluationListener(new AwaitilityOnTimeOutHandler(() -> HyperfoilResource
                        .getRunStatusDetailsResponse(context.getManagerToken(), benchmarkRuns.get(perfTestName)).then().log().all()))
                .atMost(Duration.ofMinutes(timeoutMinutes))
                .pollInterval(Duration.ofSeconds(15))
                .untilAsserted(
                        () -> HyperfoilResource
                                .getRunStatusDetailsResponse(context.getManagerToken(), benchmarkRuns.get(perfTestName))
                                .then()
                                .statusCode(200)
    ...
    public void benchmarkExecutionWasSuccessfully(String perfTestName) {
        if (benchmarkRuns.get(perfTestName) == null) {
            throw new RuntimeException("there is no benchmark run executed for " + perfTestName + " scenario");
        }
        assertThat(HyperfoilResource.getRunStatusDetailsResponse(context.getManagerToken(), benchmarkRuns.get(perfTestName))
                .then()
    ...
    @And("^The total of events received for \"([^\"]*)\" run of Bridge \"([^\"]*)\" is equal to the total of cloud events sent in:$")
    public void numberOfEventsReceivedIsEqualToEventsSent(String perfTestName, String bridgeName, DataTable parametersDatatable) {
        ...
        List<Integer> totalEventsSent = HyperfoilResource.getBenchmarkStatsDetailResponse(context.getManagerToken(), benchmarkRuns.get(perfTestName))
                .then()
                .log().ifValidationFails()
                .statusCode(200)
                .extract()
