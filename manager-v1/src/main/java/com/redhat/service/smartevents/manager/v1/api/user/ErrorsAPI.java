...
public class ErrorsAPI {
    ...
    public Response getErrors(@Valid @BeanParam QueryPageInfo queryInfo) {
        return Response.ok(PagedListResponse.fill(service.getUserErrors(queryInfo), new ErrorListResponse(), ErrorsAPI::from)).build();
    }
    ...
    public Response getError(@PathParam("id") int id) {
        ...
        return (error.isPresent() ? Response.ok(ErrorsAPI.from(error.get())) : Response.status(Status.NOT_FOUND)).build();
    }
