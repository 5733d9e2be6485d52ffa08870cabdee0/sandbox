...
public class ManagedBridgeControllerTest {
    ...
    void testCreateNewManagedBridge() {
        ...
        assertThat(managedBridge.getStatus().getConditionByType(ManagedBridgeStatus.CONFIG_MAP_AVAILABLE)).isPresent().hasValueSatisfying(c -> {
            assertThat(c.getStatus()).isEqualTo(ConditionStatus.True);
        });
        assertThat(managedBridge.getStatus().getConditionByType(ManagedBridgeStatus.KNATIVE_BROKER_AVAILABLE)).isPresent().hasValueSatisfying(c -> {
            assertThat(c.getStatus()).isEqualTo(ConditionStatus.False);
        });
        assertThat(managedBridge.getStatus().getConditionByType(ManagedBridgeStatus.AUTHORISATION_POLICY_AVAILABLE)).isPresent().hasValueSatisfying(c -> {
