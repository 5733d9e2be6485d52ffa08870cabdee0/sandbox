...
public class InputActionInvoker implements ActionInvoker {

    private static final Logger LOG = LoggerFactory.getLogger(InputActionInvoker.class);
    ...
    public InputActionInvoker(String endpoint, String cloudEventType, WebClient client, AbstractOidcClient oidcClient) {
    ...
    @Override
    public boolean requiresCloudEvent() {
        return false;
    }

    @Override
    public void onEvent(String event) {
        LOG.info("InputActionInvoker::onEvent | {} - {} | {}", endpoint, cloudEventType, event);

        HttpRequest<Buffer> request = client.postAbs(endpoint);
        String token = oidcClient.getToken();
        LOG.info("InputActionInvoker::onEvent | Token: {}", token);
        if (token != null && !"".equals(token)) {
            LOG.info("InputActionInvoker::onEvent | Token configured");
