...
public class ShardBridgesSyncAPITest {
    ...
    public void getProcessorsWithKafkaAction() {
        ...
        assertThat(processor.getStatus()).isEqualTo(ManagedEntityStatus.ACCEPTED);
    ...
    public void getProcessorsWithSendToBridgeAction() {
        ...
        BridgeDTO bridge = new BridgeDTO(bridgeId, bridgeResponse.getName(), TEST_BRIDGE_ENDPOINT, TestConstants.DEFAULT_CUSTOMER_ID, ManagedEntityStatus.READY, new KafkaConnectionDTO());
        Set<BaseFilter> filters = Collections.singleton(new StringEquals("json.key", "value"));
    ...
    public void testGetBridgesToDeploy() {
        ...
        assertThat(response.stream().filter(x -> x.getStatus().equals(ManagedEntityStatus.ACCEPTED)).count()).isEqualTo(1);
        ...
        assertThat(bridge.getStatus()).isEqualTo(ManagedEntityStatus.ACCEPTED);
    ...
    public void testGetBridgesToDelete() {
        ...
        assertThat(bridgesToDeployOrDelete.stream().filter(x -> x.getStatus().equals(ManagedEntityStatus.ACCEPTED)).count()).isZero();
        assertThat(bridgesToDeployOrDelete.stream().filter(x -> x.getStatus().equals(ManagedEntityStatus.DEPROVISION)).count()).isEqualTo(1);
    }
    ...
    public void testNotifyDeployment() {
        ...
        assertThat(bridgesToDeployOrDelete.stream().filter(x -> x.getStatus().equals(ManagedEntityStatus.ACCEPTED)).count()).isEqualTo(1);

        BridgeDTO bridge = bridgesToDeployOrDelete.get(0);
        bridge.setStatus(ManagedEntityStatus.PROVISIONING);
    ...
    public void testNotifyDeletion() {
        ...
        assertThat(bridgeResponse.getStatus()).isEqualTo(ManagedEntityStatus.DEPROVISION);

        bridge.setStatus(ManagedEntityStatus.DELETED);
        TestUtils.updateBridge(bridge).then().statusCode(200);
