...
import com.redhat.service.smartevents.manager.services.v1.BridgesService;
...
public class TestUtils {
    ...
    public static Response getBridges() {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH);
    }

    public static Response getBridgesFilterByName(String name) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + "?name=" + name);
    }
    ...
    public static Response getBridgesFilterByStatus(ManagedResourceStatus... status) {
        ...
        return jsonRequest().get(APIConstants.V1_USER_API_BASE_PATH + "?" + queryString);
    }
    ...
    public static Response getBridgesFilterByNameAndStatus(String name, ManagedResourceStatus... status) {
        String queryString = Arrays.stream(status).map(s -> "status=" + s.getValue()).collect(Collectors.joining("&"));
        return jsonRequest().get(APIConstants.V1_USER_API_BASE_PATH + "?name=" + name + "&" + queryString);
    }

    public static Response getBridge(String id) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + id);
    }

    public static Response listProcessors(String bridgeId, int page, int size) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?size=" + size + "&page=" + page);
    }

    public static Response listProcessorsFilterByName(String bridgeId, String name) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?name=" + name);
    }

    public static Response listProcessorsFilterByStatus(String bridgeId, ManagedResourceStatus... status) {
        String queryString = Arrays.stream(status).map(s -> "status=" + s.getValue()).collect(Collectors.joining("&"));
        return jsonRequest().get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?" + queryString);
    }
    ...
    public static Response listProcessorsFilterByType(String bridgeId, ProcessorType type) {
        return jsonRequest().get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?type=" + type.getValue());
    }

    public static Response listProcessorsFilterByTypeWithAnyValue(String bridgeId, String type) {
        return jsonRequest().get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?type=" + type);
    }

    public static Response listProcessorsFilterByNameAndStatus(String bridgeId, String name, ManagedResourceStatus... status) {
        String queryString = Arrays.stream(status).map(s -> "status=" + s.getValue()).collect(Collectors.joining("&"));
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?name=" + name + "&" + queryString);
    }

    public static Response listProcessorsFilterByNameAndType(String bridgeId, String name, ProcessorType type) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?name=" + name + "&type=" + type.getValue());
    }
    ...
    public static Response listProcessorsFilterByStatusAndType(String bridgeId, ManagedResourceStatus status, ProcessorType type) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors?status=" + status.getValue() + "&type=" + type.getValue());
    }

    public static Response getProcessor(String bridgeId, String processorId) {
        return jsonRequest()
                .get(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors/" + processorId);
    }

    public static Response addProcessorToBridge(String bridgeId, ProcessorRequest p) {
        return jsonRequest()
                .body(p)
                .post(APIConstants.V1_USER_API_BASE_PATH + bridgeId + "/processors/");
    }
