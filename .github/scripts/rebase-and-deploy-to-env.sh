# Env vars parameters:
# TARGET_BRANCH: dev, stable, prod
# SHA_COMMIT: the sha of the commit to rebase
# AUTHOR: The user
# GITHUB_TOKEN: The token

allowed_branch_values=("dev" "stable" "prod")

# Check for valid choices
if ! grep -q "$TARGET_BRANCH" <<< "${allowed_branch_values[@]}"; then
    printf "\U274C $TARGET_BRANCH is not a valid choice. Valid choices are dev,stable and prod."
    exit 0
fi
