...
public abstract class AbstractBridgeErrorInMemoryDAO implements BridgeErrorDAO {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractBridgeErrorInMemoryDAO.class);
    ...
    public BridgeError findErrorByIdAndType(int errorId, BridgeErrorType type) {
        ...
        if (Objects.isNull(error) || !error.getType().equals(type)) {
            throw getItemNotFoundException(String.format("Error with id %s and type %s not found in the catalog", errorId, type));
        }
        return error;
    }
