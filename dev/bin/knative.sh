...
function create_kafka_secret(){
  ...
  kubectl create secret --namespace default generic $MANAGED_KAFKA_INSTANCE_NAME-a \
...
kind: ConfigMap
metadata:
  name: $MANAGED_KAFKA_INSTANCE_NAME-a
data:
  default.topic.partitions: "10"
  default.topic.replication.factor: "3"
  bootstrap.servers: ${bootstrap_server_host}
  auth.secret.ref.name: ${MANAGED_KAFKA_INSTANCE_NAME-a}
EOF
}
