...
public abstract class AbstractAWSVaultServiceImpl implements VaultService {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractAWSVaultServiceImpl.class);
    ...
    public AbstractAWSVaultServiceImpl() {
        // CDI proxy
    }

    public AbstractAWSVaultServiceImpl(SecretsManagerAsyncClient asyncClient) {
    ...
    public Uni<Void> createOrReplace(EventBridgeSecret secret) {
        ...
        return Uni.createFrom().future(asyncClient.createSecret(createSecretRequest))
                ...
                .onFailure().transform(e -> getVaultException("Could not replace secret '%s' in AWS Vault", e))
                .invoke(() -> {
    ...
    protected abstract InternalPlatformException getVaultException(String message, Throwable e);
    ...
    public Uni<EventBridgeSecret> get(String name) {
        ...
        return Uni.createFrom().future(asyncClient.getSecretValue(
                ...
                .onFailure().transform(e -> getVaultException("Secret '%s' not found in AWS Vault", e))
                .flatMap(x -> {
