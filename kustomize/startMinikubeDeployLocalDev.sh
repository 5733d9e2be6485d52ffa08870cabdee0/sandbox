...
sed -i -E "s|(.*EVENT_BRIDGE_KAFKA_BOOTSTRAP_SERVERS=).*|\1$(rhoas kafka describe -o json | jq --raw-output '.bootstrap_server_host')|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*EVENT_BRIDGE_KAFKA_CLIENT_ID=).*|\1$( jq --raw-output '.clientID' ${CREDENTIALS_FOLDER}/${OPS_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*EVENT_BRIDGE_KAFKA_CLIENT_SECRET=).*|\1$( jq --raw-output '.clientSecret' ${CREDENTIALS_FOLDER}/${OPS_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*EVENT_BRIDGE_RHOAS_INSTANCE_API_HOST=https://admin-server-).*(/rest)|\1$(rhoas kafka describe -o json | jq --raw-output '.bootstrap_server_host')\2|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*EVENT_BRIDGE_RHOAS_SSO_MAS_CLIENT_ID=).*|\1$( jq --raw-output '.clientID' ${CREDENTIALS_FOLDER}/${ADMIN_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*EVENT_BRIDGE_RHOAS_SSO_MAS_CLIENT_SECRET=).*|\1$( jq --raw-output '.clientSecret' ${CREDENTIALS_FOLDER}/${ADMIN_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*RHOAS_OPS_ACCOUNT_CLIENT_ID=).*|\1$( jq --raw-output '.clientID' ${CREDENTIALS_FOLDER}/${OPS_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
...
sed -i -E "s|(.*MANAGED_CONNECTORS_KAFKA_BOOTSTRAP_SERVERS=).*|\1$(rhoas kafka describe -o json | jq --raw-output '.bootstrap_server_host')|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*MANAGED_CONNECTORS_KAFKA_CLIENT_ID=).*|\1$( jq --raw-output '.clientID' ${CREDENTIALS_FOLDER}/${MC_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
sed -i -E "s|(.*MANAGED_CONNECTORS_KAFKA_CLIENT_SECRET=).*|\1$( jq --raw-output '.clientSecret' ${CREDENTIALS_FOLDER}/${MC_SA_NAME}.json )|" ${KUSTOMIZE_DEPLOY_DIR}/overlays/minikube/manager/kustomization.yaml
